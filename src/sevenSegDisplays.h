...
//    SevenSegDisplays();
     ...
     *
     * @param dspUndrlHw Underlying hardware display object.
     *
     *  The underlying hardware display is the physical device used to display the information generated by this object and kept in it's buffer.
     *  As such it has physical properties that might condition some of this object characteristics -some has no practical incidence- including:
     ...
     *
     * See SevenSegDisplays(SevenSegDispHw)
     */
    ...
    ~SevenSegDisplays();
    /**
     * @brief Starts the hardware services needed to keep the display data updated and visible
     *
     * Depending on the underlying hardware several tasks and services must be configured and started
     * As these activities are hardware aware each **SevenSegDispHw** subclass implement those initialization tasks in it's own begin() method
     *
    ...
    bool blink();
    /**
     * @brief Makes the display blink the contents it is showing using the timing set by the parameters.
     *
     ...
     *
     * @retval true: If the display was not already set to blink (so now the blinking starts).
     * @retval false: The display was already set to blink, and/or one or more of the parameters passed was out of range. The blinking won't be started.
     */
     ...
     *
     * @note The clearing process involves writing a **space** character to every display port. Due to physical display characteristics this may or may not be equal to writing a '\0' to them.
     *
     */
    void clear();
    /**
     ...
     * @param labelLeft (Optional) A char, if not specified the default value, a space, will be assumed, that will be displayed in the leftmost digit of the display. The character must be one of the "displayable" characters, as listed in the **print()** method description.
    ...
    /**
     * @brief Stop and end the hardware services needed to keep the display data updated and visible
     *
     * Depending on the underlying hardware several tasks and services are configured and started
     * As these activities are hardware aware each **SevenSegDispHw** subclass implement those initialization tasks, and each subclass must implement the correct termination of those classes if required
     *
     * @return The value returned by the **SevenSegDispHw** subclass implementation of the end() method
     * @retval true: the hardware related services could be ended as needed
     * @retval false: the hardware related services could not be ended as needed
     */
    virtual bool end();
    /**
     ...
     * @retval false: The value couldn't be represented, being that the **level** parameter was out of range and/or the **label** parameter was not in the list of displayable characters. The display will be blanked.
     *
     */
